    ...
    def criar_pagina_etapa(etapa_num, etapa_data, folha_num):
        ...
        if etapa_num == 0:
            ...
            if destino_tipo == 'CP' or not ip_val:
                caminho = dados.get('cp_path', '').strip() or dados.get('pasta', '').strip()
...
def gerar_svg(dados: dict) -> list:
    ...
    user = dados.get('user', '')
    destino_tipo = dados.get('destino_tipo', 'SFTP')
    cp_path = (dados.get('cp_path', '') or dados.get('pasta', '')).strip()
    etapas = dados.get('etapas', [])
        ...
        for et in folha_ets:
            ...
            if et['tipo'] == 'SQLPLUS':
                ...
                if et_tabelas:
                    ...
                    tabelas_extra_h = max(0, bottom_tabelas - (Y + EH) + 8)
    ...
    if incluir_transferir:
        ...
        if destino_tipo == 'SFTP':
            ...
            cp_path = st.text_input(
                "Caminho da pasta (CP)", placeholder="Ex: /cifs/sh0743/SIGA_MENSAIS/REL_187_...",
                help="Sai em azul, como link, igual ao caminho do Objetivo.",
            )
