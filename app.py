...
def gerar_caracteristica_docx(dados: dict) -> bytes:
    ...
    def criar_pagina_etapa(etapa_num, etapa_data, folha_num):
        ...
        # Tabela "CAMPOS DE CLASSIFICAÇÃO"
        tbl_class = doc.add_table(rows=2, cols=8)
        tbl_class.alignment = WD_TABLE_ALIGNMENT.CENTER
        tbl_class.rows[0].cells[0].merge(tbl_class.rows[0].cells[7])
        set_cell_text(tbl_class.rows[0].cells[0], 'CAMPOS DE CLASSIFICAÇÃO', 8, True, '333333')
        headers_class = ['NUM', 'NOME DO CAMPO', 'POS.REL.', 'DIM', 'FORM', 'SEQ.', 'NUM', 'NOME DO CAMPO']
        for i, h in enumerate(headers_class):
            if i < len(tbl_class.rows[1].cells):
                set_cell_text(tbl_class.rows[1].cells[i], h, 7, False, '333333')

        doc.add_paragraph('')

        # Tabela "RELATÓRIOS"
        tbl_rel = doc.add_table(rows=2, cols=8)
        tbl_rel.alignment = WD_TABLE_ALIGNMENT.CENTER
        tbl_rel.rows[0].cells[0].merge(tbl_rel.rows[0].cells[7])
        set_cell_text(tbl_rel.rows[0].cells[0], 'RELATÓRIOS', 8, True, '333333')
        headers_rel = ['DSNAME/ETAPA', 'CS', 'SQ', 'CAMPO CONTROLE', 'CADEIA', 'CÓD. FORM.', 'CÓPIAS', 'IMPRESSORA']
        for i, h in enumerate(headers_rel):
            if i < len(tbl_rel.rows[1].cells):
                set_cell_text(tbl_rel.rows[1].cells[i], h, 7, False, '333333')
...
with col_form:
    ...
    c1, c2 = st.columns(2)
    with c1:
        data_elab = st.text_input("Data da Elaboração", value=datetime.datetime.now().strftime("%B %Y").title(), placeholder="Ex: Junho 2026")
    with c2:
    ...
    if st.button("▶▶ Gerar Característica + Fluxograma", type="primary", use_container_width=True):
        if not sql_files:
            ...
            for idx, sql_file in enumerate(sql_files):
                ...
                if len(sql_files) > 1:
                    # Usa letra A, B, C... para diferenciar
                    sufixo = chr(65 + idx) if idx < 26 else str(idx + 1)
                    if not cod_atual[-1].isalpha():
...
with col_output:
    ...
    if st.session_state.resultados:
        for res in st.session_state.resultados:
            ...
            with dl_cols[0]:
                st.download_button(
                    label=f"⬇ Característica_{cod}.docx",
                    data=res['docx_bytes'],
