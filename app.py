...
def gerar_caracteristica_docx(dados: dict) -> bytes:
    ...
    def add_texto(text, size=9, bold=False, underline=False, space_after=5, space_before=0):
        p = doc.add_paragraph()
        run = p.add_run(text)
        run.font.size = Pt(size)
        run.font.bold = bold
    ...
    def add_link_texto(text, size=9, space_after=5, space_before=0):
    ...
    def add_bullet(text, size=9, space_after=4):
    ...
    def criar_pagina_etapa(etapa_num, etapa_data, folha_num):
        ...
        for lbl in ['☐ SORT', '☐ MERGE']:
            ...
            rc = pc.add_run(lbl)
            rc.font.size = Pt(9); rc.font.name = 'Arial Narrow'; rc.font.color.rgb = RGBColor.from_string(PRETO)
        ...
        r1_.font.size = Pt(9); r1_.font.name = 'Arial Narrow'; r1_.font.color.rgb = RGBColor.from_string(PRETO)
        r2_ = p.add_run(' = Campo1')
        r2_.font.size = Pt(9); r2_.font.name = 'Arial Narrow'; r2_.font.color.rgb = RGBColor.from_string(PRETO)
        ...
        if etapa_num == 0:
            ...
            add_texto('OBJETIVO:', bold=True, underline=True, space_after=4)
            ...
            add_texto('EXECUTAR SCRIPT SQL:', bold=True, underline=True, space_after=4)
            ...
            for cmd in ['export LANG=pt_BR.ISO-8859-1',
                ...
                add_texto(cmd, space_after=3)

        elif etapa_data.get('tipo') == 'COMPACTAR':
            add_texto('COMPACTAR ARQUIVO:', bold=True, underline=True, space_after=4)
            add_bullet(etapa_data.get('inp', ''))
            ...
            add_texto(titulo, bold=True, underline=True, space_after=4)
            for arq in arquivos:
                add_bullet(arq)
            ...
            if destino_tipo == 'CP' or not ip_val:
                ...
                add_texto('Disponibilizar os arquivos compactados, na pasta (CP):', space_before=6)
...
def gerar_svg(dados: dict) -> list:
    ...
    def draw_header(els, Y, fnum):
        ...
        els.append(f'<text x="{C1//2}" y="{Y+RH1//2+6}" font-size="16" font-weight="bold" text-anchor="middle" fill="{PRETO}" font-family="{FNT}">PRODAM</text>')
        els.append(f'<text x="{C1+C2//2}" y="{Y+RH1//2+6}" font-size="13" font-weight="bold" text-anchor="middle" fill="{PRETO}" font-family="{FNT}">FLUXOGRAMA DE ROTINA DE OPERAÇÃO</text>')
        els.append(f'<text x="{C1+C2+C3//2}" y="{Y+14}" font-size="8" text-anchor="middle" fill="{PRETO}" font-family="{FNT}">CÓDIGO DA ROTINA:</text>')
        els.append(f'<text x="{C1+C2+C3//2}" y="{Y+29}" font-size="12" font-weight="bold" text-anchor="middle" fill="{PRETO}" font-family="{FNT}">{ex(cod)} ({ex(amb)})</text>')
        ...
        els.append(f'<text x="8" y="{y2+11}" font-size="8" fill="{PRETO}" font-family="{FNT}">SISTEMA:</text>')
        els.append(f'<text x="8" y="{y2+RH2-4}" font-size="11" font-weight="bold" fill="{PRETO}" font-family="{FNT}">{ex(sis)}</text>')
        els.append(f'<text x="{SW1+8}" y="{y2+11}" font-size="8" fill="{PRETO}" font-family="{FNT}">SUBSISTEMA:</text>')
        els.append(f'<text x="{SW1+8}" y="{y2+RH2-4}" font-size="11" font-weight="bold" fill="{PRETO}" font-family="{FNT}">{ex(sub)}</text>')
        ...
        els.append(f'<text x="8" y="{y3+12}" font-size="8" fill="{PRETO}" font-family="{FNT}">DENOMINAÇÃO DO PROGRAMA</text>')
        els.append(f'<text x="8" y="{y3+28}" font-size="11" font-weight="bold" fill="{PRETO}" font-family="{FNT}">{ex(den)}</text>')
        els.append(f'<text x="{DW+8}" y="{y3+12}" font-size="8" fill="{PRETO}" font-family="{FNT}">DATA DA ELABORAÇÃO:</text>')
        els.append(f'<text x="{DW+8}" y="{y3+28}" font-size="10" fill="{PRETO}" font-family="{FNT}">{ex(dat)}</text>')
        els.append(f'<text x="{DW+FW3+8}" y="{y3+12}" font-size="8" fill="{PRETO}" font-family="{FNT}">FOLHA:</text>')
        els.append(f'<text x="{DW+FW3+8}" y="{y3+28}" font-size="12" font-weight="bold" fill="{PRETO}" font-family="{FNT}">{str(fnum).zfill(2)}</text>')
    ...
    def draw_oval(els, Y, nome):
        ...
        for i, linha in enumerate(linhas):
            els.append(f'<text x="{W//2}" y="{start_y + i*line_h}" font-size="10" text-anchor="middle" fill="{GRY}" font-family="{FNT}">{ex(linha)}</text>')
        return Y + fh
    ...
    def draw_cp_path(els, Y, path):
        ...
        els.append(f'<text x="{W//2}" y="{Y+14}" font-size="9" text-anchor="middle" fill="{LGY}" font-family="{FNT}">Disponibilizar na pasta (CP):</text>')
        start_y = Y + 28
        for i, linha in enumerate(linhas):
            els.append(f'<text x="{W//2}" y="{start_y + i*line_h}" font-size="9" text-anchor="middle" fill="{AZUL}" text-decoration="underline" font-family="{FNT}">{ex(linha)}</text>')
        return Y + fh + 10
    ...
    for fi, folha_ets in enumerate(folhas_etapas):
        ...
        for et in folha_ets:
            ...
            if et['tipo'] == 'SQLPLUS':
                ...
                et_tabelas = et.get('tabelas', [])
                if et_tabelas:
                    tx = CX + CW + 12
                    ty = CY - 2
                    els.append(f'<text x="{tx}" y="{ty}" font-size="8" fill="{LGY}" font-family="{FNT}">Tabelas:</text>')
                    for ti, tn in enumerate(et_tabelas):
                        els.append(f'<text x="{tx}" y="{ty + 11 + ti*11}" font-size="8" fill="{GRY}" font-family="{FNT}">{ex(tn)}</text>')
                    bottom_tabelas = ty + 11 + 11 * len(et_tabelas)
                    # Não estica o espaço entre etapas — só guarda o quanto a lista
                    # "vaza" pra baixo, pra folha inteira crescer no final se precisar.
                    folha_extra_bottom = max(folha_extra_bottom, bottom_tabelas)
