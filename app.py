...
def gerar_caracteristica_docx(dados: dict) -> bytes:
    ...
    def add_texto(text, size=9, bold=False, underline=False, space_after=9, space_before=0):
        p = doc.add_paragraph()
        run = p.add_run(text)
        run.font.size = Pt(size)
        run.font.bold = bold
    ...
    def add_link_texto(text, size=9, space_after=9, space_before=0):
    ...
    def add_bullet(text, size=9, space_after=7):
...
def gerar_svg(dados: dict) -> list:
    ...
    def draw_flag(els, Y, texto, tabelas_lista=None):
        ...
        for i, linha in enumerate(linhas):
            els.append(f'<text x="{X+fw*0.46}" y="{start_y + i*line_h}" font-size="10" text-anchor="middle" fill="{GRY}" font-family="{FNT}">{ex(linha)}</text>')
