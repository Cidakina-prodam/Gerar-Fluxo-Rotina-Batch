...
def gerar_caracteristica_docx(dados: dict) -> bytes:
    ...
    # ─── HELPERS ────────────────────────────────────────────────
    def set_borders(cell, color=PRETO, sz=4):
    ...
    def criar_pagina_etapa(etapa_num, etapa_data, folha_num):
        ...
        if etapa_num == 0:
            ...
            add_texto('IMPORTANTE:', bold=True, underline=True, space_after=4)
            add_texto('COMANDOS NECESSÁRIOS ANTES DE INICIAR O SQLPLUS:')
            ...
            if etapa_data.get('out'):
                ...
                add_texto(etapa_data.get('out', ''))
                if '&' in etapa_data.get('out', ''):
                    add_texto("Onde: &x = 'DD-MM-YYYY'")

        elif etapa_data.get('tipo') == 'COMPACTAR':
            add_texto('COMPACTAR ARQUIVO:', bold=True, underline=True, space_after=4)
            add_texto(etapa_data.get('out', etapa_data.get('inp', '') + '.zip'))

        elif etapa_data.get('tipo') == 'TRANSFERIR':
            add_texto('TRANSFERIR ARQUIVO:', bold=True, underline=True, space_after=4)
            add_texto(etapa_data.get('inp', ''))
            add_texto('Disponibilizar os arquivos compactados, na pasta (CP):', space_before=6)
            add_texto('Destino do arquivo gerado (servidor SFTP):', bold=True, underline=True, space_after=4)
            sftp_items = [
                f'- IP: {dados.get("ip", "")}',
        ...
        NCOLS_CLASS = 12
        tbl_class = doc.add_table(rows=3, cols=NCOLS_CLASS)
        ...
        NCOLS_REL = 9
        tbl_rel = doc.add_table(rows=3, cols=NCOLS_REL)
...
def gerar_svg(dados: dict) -> list:
    ...
    # para fins de paginação — o SFTP deve ficar na mesma folha
...
with col_form:
    ...
    objetivo_txt = st.text_area(
        ...
        height=100,
        help="Vai direto no corpo da Característica, na ETAPA 00, sob o título OBJETIVO. "
             "Use uma linha por parágrafo (ex: descrição + caminho/pasta de destino).",
    )
    ...
    if st.button("▶▶ Gerar Característica + Fluxograma", type="primary", use_container_width=True):
        if not sql_files:
            ...
            # Ordenar scripts por nome para manter sequência (01_, 02_, ...)
            sql_files_sorted = sorted(sql_files, key=lambda f: f.name)

            for idx, sql_file in enumerate(sql_files_sorted):
