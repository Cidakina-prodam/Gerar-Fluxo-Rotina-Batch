...
with col_output:
    ...
    if st.session_state.svgs:
        ...
        if len(svgs) > 1:
            # Combina num único SVG válido empilhando as folhas verticalmente
            import re
            W = 700
            combined_els = []
            total_h = 0
            ...
            st.download_button(
                label=f"⬇ Baixar TODAS as folhas ({len(svgs)} folhas em 1 SVG)",
