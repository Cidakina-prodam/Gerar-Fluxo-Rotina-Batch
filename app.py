...
def gerar_fluxograma_docx(svgs: list, cod: str = 'fluxograma') -> bytes:
    """Converte lista de SVGs do fluxograma em um .docx com imagens embutidas."""
    import cairosvg
    from docx import Document
    ...
    # Configurar margens estreitas e orientação retrato
    for section in doc.sections:
    ...
    for i, svg_str in enumerate(svgs):
        # Converter SVG → PNG em memória
        png_bytes = cairosvg.svg2png(
            bytestring=svg_str.encode('utf-8'),
            output_width=1400,  # 2x para boa resolução
        )
        img_stream = io.BytesIO(png_bytes)

        # Inserir imagem no documento (largura = largura útil da página)
        doc.add_picture(img_stream, width=Cm(18))

        # Quebra de página entre folhas (exceto na última)
        if i < len(svgs) - 1:
            doc.add_page_break()
    ...
    if st.button("▶▶ Gerar Característica + Fluxograma", type="primary", use_container_width=True):
        if not sql_files:
            ...
            for idx, sql_file in enumerate(sql_files):
                ...
                if len(sql_files) > 1:
                    # Usa letra A, B, C... para diferenciar — sempre aplica,
                    # independente de o código já terminar em letra
