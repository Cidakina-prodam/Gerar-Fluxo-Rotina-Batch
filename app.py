...
def gerar_svg(dados: dict) -> str:
    ...
    W = 700
    BLU, BLU2, GRY, LGY, BRD = '#1F4E79', '#2E75B6', '#333', '#888', '#333'
    FNT = 'Arial Narrow,Arial,sans-serif'
    Y = 0
    els = []
    ex = escape_svg

    # ─── CABEÇALHO TABELA PRODAM (3 linhas) ──────────────────────
    # Linha 1: PRODAM | FLUXOGRAMA | CÓDIGO
    C1, C2, C3 = 120, 380, 200
    RH1, RH2, RH3 = 36, 24, 36
    HH = RH1 + RH2 + RH3
    bw = 1.5  # largura da borda

    # Fundo completo
    els.append(f'<rect x="0" y="0" width="{W}" height="{HH}" fill="{BLU}"/>')
    # Borda externa
    els.append(f'<rect x="0" y="0" width="{W}" height="{HH}" fill="none" stroke="#fff" stroke-width="{bw}"/>')

    # Linha 1
    els.append(f'<line x1="{C1}" y1="0" x2="{C1}" y2="{RH1}" stroke="#fff" stroke-width="{bw}"/>')
    els.append(f'<line x1="{C1+C2}" y1="0" x2="{C1+C2}" y2="{RH1}" stroke="#fff" stroke-width="{bw}"/>')
    els.append(f'<line x1="0" y1="{RH1}" x2="{W}" y2="{RH1}" stroke="#fff" stroke-width="{bw}"/>')
    els.append(f'<text x="{C1//2}" y="{RH1//2+5}" font-size="14" font-weight="bold" text-anchor="middle" fill="#fff" font-family="{FNT}">PRODAM</text>')
    els.append(f'<text x="{C1+C2//2}" y="{RH1//2+5}" font-size="12" font-weight="bold" text-anchor="middle" fill="#fff" font-family="{FNT}">FLUXOGRAMA DE ROTINA DE OPERAÇÃO</text>')
    els.append(f'<text x="{C1+C2+C3//2}" y="{RH1//2-2}" font-size="8" text-anchor="middle" fill="#9BB8D8" font-family="{FNT}">CÓDIGO DA ROTINA:</text>')
    els.append(f'<text x="{C1+C2+C3//2}" y="{RH1//2+12}" font-size="12" font-weight="bold" text-anchor="middle" fill="#fff" font-family="{FNT}">{ex(cod)} ({ex(amb)})</text>')
    ...
    els.append(f'<text x="{W-C3+10}" y="{y3+27}" font-size="10" font-weight="bold" fill="#fff" font-family="{FNT}">{ex(dat)}    FOLHA: 01</text>')

    Y = HH + 20

    TIPOS_STYLE = {
        'SQLPLUS':    {'stroke': '#333'},
        'COMPACTAR':  {'stroke': '#333'},
        'TRANSFERIR': {'stroke': '#333'},
        'SHELL':      {'stroke': '#333'},
        'EMAIL':      {'stroke': '#333'},
        'CUSTOM':     {'stroke': '#333'},
    }

    # ─── HELPER: desenha oval de arquivo ─────────────────────────
    def draw_oval(y, nome):
    ...
    # ─── HELPER: desenha seta vertical ───────────────────────────
    def draw_arrow(y, length=24, color=GRY):
        mid = y + length - 8
        els.append(f'<line x1="{W//2}" y1="{y}" x2="{W//2}" y2="{mid}" stroke="{color}" stroke-width="1.2"/>')
        els.append(f'<polygon points="{W//2},{y+length} {W//2-5},{mid} {W//2+5},{mid}" fill="{color}"/>')
        return length

    # ─── ETAPAS ──────────────────────────────────────────────────
    prev_out = None  # para evitar oval duplicado
    ...
    for idx, et in enumerate(etapas):
        ...
        # Arquivo entrada (oval) — só mostra se diferente do out anterior
        inp = et.get('inp', '')
        ...
        if et['tipo'] == 'SQLPLUS':
            # Rotina (retângulo arredondado, esquerda)
            RW, RH = 90, 48
            RX, RY = EX - RW - 30, Y + (EH - RH) // 2
            ...
            CW, CRH = 90, 56
            CX, CY = EX + EW + 30, Y + (EH - CRH) // 2
            CRY_EL = 10
            els.append(f'<ellipse cx="{CX+CW//2}" cy="{CY+CRY_EL}" rx="{CW//2}" ry="{CRY_EL}" fill="none" stroke="{BRD}" stroke-width="1.2"/>')
            els.append(f'<rect x="{CX}" y="{CY+CRY_EL}" width="{CW}" height="{CRH-CRY_EL*2}" fill="#fff" stroke="{BRD}" stroke-width="1.2"/>')
            els.append(f'<ellipse cx="{CX+CW//2}" cy="{CY+CRH-CRY_EL}" rx="{CW//2}" ry="{CRY_EL}" fill="none" stroke="{BRD}" stroke-width="1.2"/>')
        ...
        # Caixa da etapa (retângulo com divisória)
        els.append(f'<rect x="{EX}" y="{Y}" width="{EW}" height="{EH}" fill="none" stroke="{BRD}" stroke-width="1.8"/>')
        els.append(f'<line x1="{EX}" y1="{Y+EH1}" x2="{EX+EW}" y2="{Y+EH1}" stroke="{BRD}" stroke-width="1"/>')
        els.append(f'<text x="{EX+EW//2}" y="{Y+EH1-8}" font-size="13" font-weight="bold" text-anchor="middle" fill="{GRY}" font-family="{FNT}">ETAPA {str(et["num"]).zfill(2)}</text>')
        els.append(f'<text x="{EX+EW//2}" y="{Y+EH1+EH2-9}" font-size="11" text-anchor="middle" fill="{GRY}" font-family="{FNT}">{ex(et.get("titulo","") or et["tipo"])}</text>')
        Y += EH
    ...
    # ─── BLOCO SFTP (cilindro grande) ────────────────────────────
    if ip:
        Y += draw_arrow(Y, 28, '#333')
        Y += 4
        CW, CRH = 260, 120
        CX = (W - CW) // 2
        CRY_EL = 16
        # Cilindro com elipses topo e base
        els.append(f'<ellipse cx="{CX+CW//2}" cy="{Y+CRY_EL}" rx="{CW//2}" ry="{CRY_EL}" fill="none" stroke="{BRD}" stroke-width="1.5"/>')
        els.append(f'<rect x="{CX}" y="{Y+CRY_EL}" width="{CW}" height="{CRH-CRY_EL*2}" fill="#fff" stroke="{BRD}" stroke-width="1.5"/>')
        els.append(f'<ellipse cx="{CX+CW//2}" cy="{Y+CRH-CRY_EL}" rx="{CW//2}" ry="{CRY_EL}" fill="none" stroke="{BRD}" stroke-width="1.5"/>')
        # Textos dentro do cilindro
        els.append(f'<text x="{CX+CW//2}" y="{Y+38}" font-size="12" font-weight="bold" text-anchor="middle" fill="{GRY}" font-family="{FNT}">SFTP — Servidor Externo</text>')
        els.append(f'<text x="{CX+CW//2}" y="{Y+55}" font-size="10" text-anchor="middle" fill="{GRY}" font-family="{FNT}">IP: {ex(ip)}  |  Porta: {ex(porta)}</text>')
        els.append(f'<text x="{CX+CW//2}" y="{Y+70}" font-size="10" text-anchor="middle" fill="{GRY}" font-family="{FNT}">Usuário: {ex(user)}</text>')
        els.append(f'<text x="{CX+CW//2}" y="{Y+85}" font-size="9" text-anchor="middle" fill="{LGY}" font-family="{FNT}">{ex(pasta)}</text>')
        Y += CRH + 10
    ...
    els.append(f'<text x="{W-10}" y="{Y+12}" font-size="8" text-anchor="end" fill="{LGY}" font-family="{FNT}">FOLHA 01/01</text>')
    Y += 20

    if tabelas:
        els.append(f'<text x="10" y="{Y+10}" font-size="8" fill="{LGY}" font-family="{FNT}">Tabelas Oracle: {ex(", ".join(tabelas))}</text>')
        Y += 18
